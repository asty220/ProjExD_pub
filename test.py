...
class Bomb(pg.sprite.Sprite):
    ...
    # C0B21048 
    def update(self, screen):                                               #updateを追加(兼田)
...
def scores(sc):                                                              #ゲームオーバー画面を表示するための関数
    ...
    while True:
        score_screen = Screen("sozai/pg_bg.jpg", (1600, 900), ("game over")) #Screenクラスを用いてscoreのスクリーンを表示(熊田)
...
def start():            #スタート画面の表示
    timecount=0
    while True:
        start_screen = Screen("sozai/pg_bg.jpg", (1600, 900), "Start") #Screenクラスを用いてscoreのスクリーンを表示(熊田)
        start_screen.disp.blit(start_screen.surface,start_screen.rect)
...
def check_bound(sc_r, obj_r):                               #check_boundを追加(兼田)
